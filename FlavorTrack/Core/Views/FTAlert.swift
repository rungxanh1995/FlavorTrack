...
private extension FTAlert {
	...
	private func configAlertButton() {
		...
		alertButton.setTitle(
			NSLocalizedString(buttonTitleString ?? "OK", comment: "The text of the alert button title"),
			for: .normal)
	...
	private func configMessageLabel() {
		...
		messageLabel.text = NSLocalizedString(
			messageString ?? "Unable to complete request",
			comment: "The text of the alert message")
