...
private extension BusinessListVC {
	...
	private func configSearchBar() {
		...
		searchController.searchBar.placeholder = NSLocalizedString(
			"Search a name",
			comment: "The placeholder of search bar in Business list controller")
	...
	private func getBusinessList(for businessType: String, near location: String) {
		...
		Task {
			do {
				let result: RawServerResponse = try await BusinessDataService
					.shared
					.fetchData(nearby: location, businessType: businessType)
