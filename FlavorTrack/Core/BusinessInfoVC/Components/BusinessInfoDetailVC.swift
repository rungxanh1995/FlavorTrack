...
private extension BusinessInfoDetailVC {
	...
	private func configUIElements() {
		...
		locationLabel.text = business.location?.displayAddress?.joined(separator: ", ") ??
			NSLocalizedString("No address information available", comment: "")
		...
		yelpAttrString.addAttribute(.link, value: business.yelpUrl,
									range: NSRange(location: 0, length: yelpAttrString.length))
